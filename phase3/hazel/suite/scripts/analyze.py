...
def comparison_plot(allrows):
    import matplotlib.pyplot as plt
    hosts=sorted({host for host,row in allrows})
    columns=min(4,max(1,len(hosts))); rows_count=math.ceil(len(hosts)/columns)
    fig,axes=plt.subplots(rows_count,columns,figsize=(3.5*columns,3.5*rows_count),sharex=True,sharey=True,squeeze=False)
    sources=[]
    ...
    for ax in axes.flat[len(hosts):]:ax.set_visible(False)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--final',action='store_true');ap.add_argument('--no-plots',action='store_true');ap.add_argument('--hosts',nargs='+');args=ap.parse_args()
    cfg=json.loads((ROOT/'config/phase1.json').read_text())
    ...
    if set(hosts)-set(cfg['hosts']):ap.error('Unknown Hazel constraint')
    for host in hosts:
        ...
        info['course_cpu_model']=COURSE_MODELS.get(host,host+' constraint; exact allocated model recorded separately')
