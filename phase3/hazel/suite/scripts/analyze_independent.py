...
def main():
    ...
    for host in hosts:
        ...
        variant=ROOT/'machines'/host/'independent-load-diagnostic-v2'
        diagnostic_variant=(variant/'full-finished.json').exists()
        if diagnostic_variant:out=variant
        ...
        for build in (out/'build').iterdir():
            env=next(json.loads(p.read_text()) for p in list(out.glob('environment-*.json'))+list(out.glob('environment-history/job-*.json')))
        ...
        if diagnostic_variant:
            policy=json.loads((out/'diagnostic-policy.json').read_text())
            assert policy['batch']==cohorts['batch']==plan['initial_batch']
            assert policy['samples']==plan['samples']
            assert policy['calibration_gate']=='not performed: user-requested diagnostic variant'
            assert json.loads((out/'full-finished.json').read_text())['calibration_gate']=='not performed'
            fraction=None;calibration={'records':[]};g=[]
        else:
        ...
        result=dict(host=host,unit=next(d['measurement']['unit'] for d in full if d['parameters']['mode']=='chase')+'/load',batch=cohorts['batch'],qualified_pairs=len(pairs),planned_pairs=6,
            ...
            timer_fraction=fraction,calibration_cpu=g[0]['selected']['cpu'] if g else None,calibration_gate='not performed (approved diagnostic variant)' if diagnostic_variant else 'passed',calibration_records=calibration['records'],
    ...
    configured=json.loads((ROOT/'config/phase1.json').read_text())['hosts']
    write_json(dest/'verification.json',dict(time=now(),passed=True,all_configured_targets=set(hosts)==set(configured),hosts=hosts,
        ...
        checks=['All raw counts/hashes/full statistics/block medians','Recomputed noise flags','Two idle windows and SMT siblings',
                ...
                'All six seed/order cohorts','Timer overhead <=5% for original variant; explicit no-calibration policy verified for approved diagnostic variant','Runtime pointer identities'],inputs=inputs))
