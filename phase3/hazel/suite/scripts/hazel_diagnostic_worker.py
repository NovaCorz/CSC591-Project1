...
import independent_diagnostic_worker as independent_worker
...
def independent(constraint):
    out = ROOT.parent / 'runs' / constraint / 'independent-load-diagnostic-v2'
    out.mkdir(parents=True, exist_ok=True)
    config = json.loads((ROOT / 'config/phase1.json').read_text())
...
def main():
    ...
    out = ROOT.parent / 'runs' / constraint / (
        'page_control' if args.kind == 'page_control' else 'independent-load-diagnostic-v2')
    out.mkdir(parents=True, exist_ok=True)
