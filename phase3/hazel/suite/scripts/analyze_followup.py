...
OUT=ROOT.parent/'analysis/followup'

def plan_for(host):
    path=ROOT.parent/'analysis'/host/'followup-plan.json'
    if path.exists():return json.loads(path.read_text())
    return json.loads((ROOT/'config/followup-plan.json').read_text())['hosts'][host]
...
def revised_cache_table(result):
    ...
    for role,index in [('L2-like candidate',0),('LLC-like candidate',1)]:
        ...
        if capacity and agreement:
            spacing=plan_for(host)['spatial_candidate']
...
def host_analysis(host):
    ...
    for family in ('spatial_alignment','spatial_confirmation','capacity_control','capacity_refinement','capacity_regular','empirical_conflict','cross_core_reload','same_core_reload','target_hot','followup_overhead','followup_hot_batch','l2_conflict','l2_upper_calibration','l2_saturation','l2_strong_conflict','cross_core_reload_final','same_core_reload_final','target_hot_final'):
    ...
    finished={stage:json.loads((root/(stage+'-finished.json')).read_text()) if (root/(stage+'-finished.json')).exists() else None for stage in ('spatial','capacity','confirm','conflict','cross','l2','l2confirm','inclusion_final')}
    ...
    if not missing:
        ...
        expected_final={(w,seed,offset) for w in plan['cross_footprints'] for seed in (5924027,5924028) for offset in (0,1)}
        actual_final={(d['parameters']['bytes'],d['parameters']['seed'],d['parameters']['offset']) for d in rows if d['family']=='cross_core_reload_final'}
        if expected_final-actual_final:issues.append(dict(reason='missing post-refinement inclusion controls',configurations=sorted(expected_final-actual_final)))
    post_rows=[]
    for row in rows:
        if row['family'] in ('cross_core_reload_final','same_core_reload_final','target_hot_final'):
            copy=dict(row);copy['family']=row['family'].removesuffix('_final');post_rows.append(copy)
    result=dict(host=host,time=now(),verified_points=len(rows),noisy_points=sum(d['qualification']=='noisy' for d in rows),
        ...
        capacity=capacity_inference(rows),prefetch=prefetch_controls(rows),conflict=conflict_inference(rows,root),cross_core=cross_inference(rows),
        post_refinement_inclusion=cross_inference(post_rows),l2_specific=l2_inference(rows,root),l2_confirmation=strong_lower_inference(rows,root))
...
def main():
    ...
    state=dict(time=now(),phase='I timing only',hosts=results,command=[sys.executable]+sys.argv,
        ...
        complete=all(d['acquisition_complete'] for d in results.values()),
        all_configured_targets=set(hosts)==set(allowed))
