#!/usr/bin/env python3
"""Diagnostic variant without a timer-calibration gate; retains functional/idle/noise controls."""
import argparse
import fcntl
import hashlib
import json
...
FILES = ['src/cache_bench.c','src/independent_load.c','scripts/common.py','scripts/worker.py',
         'scripts/independent_diagnostic_worker.py','scripts/hazel_diagnostic_worker.py','config/phase1.json','config/independent-load.json']
...
def run(stage,out,cfg,rows,plan,digest):
    ...
    batch=plan['initial_batch']
    write_json(out/'diagnostic-policy.json',dict(time=now(),source_digest=digest,
        calibration_gate='not performed: user-requested diagnostic variant',
        batch=batch,samples=plan['samples'],
        interpretation='Uncorrected native timer ticks/load; dependent versus independent throughput comparison. Not a physical cache-hit latency estimate.',
        controls='Functional checks, assembly dependency audit, idle affinity, sample count, raw integrity and noise flags retained'))
    cohorts=[]
    ...
    write_json(out/'full-finished.json',dict(time=now(),source_digest=digest,calibration_gate='not performed',cohorts=len(cohorts),qualified=sum(bool(c['selected']) for c in cohorts)))
